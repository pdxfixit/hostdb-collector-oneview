...
# general
APP_NAME            = hostdb-collector-oneview
APP_VER             = 0.1
CIRCLE_WORKFLOW_ID ?= ""
WORK_DIR            = $(shell pwd)
...
sample_data: ## output collected data to a file
	if [ "$$(docker images -q $(APP_NAME))" == "" ]; then $(MAKE) build; fi
	time docker run $(DOCKER_RUN_OPTIONS) -e HOSTDB_COLLECTOR_ONEVIEW_COLLECTOR_SAMPLE_DATA=true -v `pwd`/sample-data:/sample-data $(APP_NAME)
